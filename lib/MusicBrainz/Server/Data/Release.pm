...
sub filter_barcode_changes {
    ...
    return @{
        $self->c->sql->select_list_of_hashes(
            'SELECT DISTINCT change.release, change.barcode, change.edit_note
             FROM (VALUES ' . join(', ', ("(?::uuid, ?, ?)") x @barcodes) . ') change (release, barcode, edit_note)
            ...
            map { $_->{release}, $_->{barcode}, $_->{edit_note} } @barcodes
        )
    };
}
